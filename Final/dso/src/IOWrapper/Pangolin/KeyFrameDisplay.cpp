...
void KeyFrameDisplay::exportPointCloud(std::vector<Eigen::Vector3f>& points, std::vector<Eigen::Vector3f>& colors,
                                        float scaledTH, float absTH, int mode, float minBS, int sparsity)
{
	points.clear();
	colors.clear();
	...
	// Filter points similar to refreshPC
	for(int i=0;i<numSparsePoints;i++)
	{
		if(mode==1 && originalInputSparse[i].status != 1 && originalInputSparse[i].status!= 2) continue;
		if(mode==2 && originalInputSparse[i].status != 1) continue;
		if(mode>2) continue;
		...
		if(originalInputSparse[i].relObsBaseline < minBS)
			continue;

		// Process each pattern point
		for(int pnt=0;pnt<patternNum;pnt++)
		{
			if(sparsity > 1 && rand()%sparsity != 0) continue;
			int dx = patternP[pnt][0];
			int dy = patternP[pnt][1];

			// Convert to 3D point in camera frame
			Eigen::Vector3f pt_cam;
			pt_cam[0] = ((originalInputSparse[i].u+dx)*fxi + cxi) * depth;
			pt_cam[1] = ((originalInputSparse[i].v+dy)*fyi + cyi) * depth;
			pt_cam[2] = depth;

			// Transform to world frame
			Eigen::Matrix4f pose = camToWorld.matrix().cast<float>();
			Eigen::Vector4f pt_cam_hom(pt_cam[0], pt_cam[1], pt_cam[2], 1.0f);
			Eigen::Vector4f pt_world_hom = pose * pt_cam_hom;
			Eigen::Vector3f pt_world = pt_world_hom.head<3>();
			...
			{
				if(originalInputSparse[i].status==0)
					color = Eigen::Vector3f(0, 255, 255);
				else if(originalInputSparse[i].status==1)
					color = Eigen::Vector3f(0, 255, 0);
				else if(originalInputSparse[i].status==2)
					color = Eigen::Vector3f(0, 0, 255);
				else if(originalInputSparse[i].status==3)
					color = Eigen::Vector3f(255, 0, 0);
				else
					color = Eigen::Vector3f(255, 255, 255);
			}
			else
			{
				unsigned char c = originalInputSparse[i].color[pnt];
				color = Eigen::Vector3f(c, c, c);
			}
