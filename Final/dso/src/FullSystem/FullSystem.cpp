...
{
	if(optimized == nullptr || toOptimize == nullptr) return;
...
void FullSystem::activatePointsMT()
{
	...
	for(FrameHessian* host : frameHessians)		// go through all active frames
	{
		if(host == nullptr || host == newestHs) continue;
...
//			(int)toOptimize.size(), immature_deleted, immature_notReady, immature_needMarg, immature_want, immature_margskip);
	...
	for(unsigned k=0;k<toOptimize.size() && k<optimized.size();k++)
	{
		PointHessian* newpoint = optimized[k];
		ImmaturePoint* ph = toOptimize[k];
		
		if(ph == nullptr) continue; // Safety check

		if(newpoint != 0 && newpoint != (PointHessian*)((long)(-1)))
		{
			if(newpoint->host == nullptr) {
				delete newpoint;
				delete ph;
				continue;
			}
			...
			for(PointFrameResidual* r : newpoint->residuals)
				...
				if(newpoint->efPoint == 0) {
					printf("WARNING: efPoint is 0 after insertPoint!\n");
				}
				delete ph;
			} catch (...) {
...
void FullSystem::flagPointsForRemoval()
{
	...
	for(FrameHessian* host : frameHessians)		// go through all active frames
	{
		if(host == nullptr) continue; // Safety check
		
		for(unsigned int i=0;i<host->pointHessians.size();i++)
		{
			PointHessian* ph = host->pointHessians[i];
			if(ph==0 || ph==nullptr) continue;
...
void FullSystem::deliverTrackedFrame(FrameHessian* fh, bool needKF)
{
	...
	else
	{
		...
		if(needKF && fh != nullptr && fh->shell != nullptr && fh->shell->trackingRef != nullptr) {
...
void FullSystem::mappingLoop()
{
	...
	while(runMapping)
	{
		...
		{
			if(setting_realTimeMaxKF || (frameHessians.size() > 0 && needNewKFAfter >= frameHessians.back()->shell->id))
			{
				lock.unlock();
				try {
				makeKeyFrame(fh);
...
{
	if(fh == nullptr) return; // Safety check
	
	// needs to be set by mapping thread. no lock required since we are in mapping thread.
	{
		boost::unique_lock<boost::mutex> crlock(shellPoseMutex);
		if(fh->shell == nullptr || fh->shell->trackingRef == nullptr) {
			printf("ERROR: makeNonKeyFrame: invalid shell or trackingRef!\n");
			delete fh;
			return;
		}
		fh->shell->camToWorld = fh->shell->trackingRef->camToWorld * fh->shell->camToTrackingRef;
...
void FullSystem::makeKeyFrame( FrameHessian* fh)
{
	...
	// needs to be set by mapping thread
	{
		boost::unique_lock<boost::mutex> crlock(shellPoseMutex);
		if(fh->shell == nullptr || fh->shell->trackingRef == nullptr) {
			printf("ERROR: makeKeyFrame: invalid shell or trackingRef!\n");
			return;
		}
		fh->shell->camToWorld = fh->shell->trackingRef->camToWorld * fh->shell->camToTrackingRef;
		fh->setEvalPT_scaled(fh->shell->camToWorld.inverse(),fh->shell->aff_g2l);
	...
	{
		if(fh1 == nullptr || fh1 == fh) continue;
		if(fh1->pointHessians.empty()) continue;
	...
	try {
		...
		printf("ERROR: optimize failed in makeKeyFrame!\n");
		rmse = 1e10; // Set high error to indicate failure
	}
	...
	{
		if(frameHessians[i] == nullptr) continue; // Safety check
		if(frameHessians[i]->flaggedForMarginalization)
		{
			try {
				marginalizeFrame(frameHessians[i]);
				i=0; // Restart loop after marginalization
			} catch (...) {
				printf("ERROR: marginalizeFrame failed!\n");
