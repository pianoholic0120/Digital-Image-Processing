...
void Undistort::readFromFile(const char* configFileName, int nPars, std::string prefix)
{
    ...
    // Read l1, skipping comment lines
    ...
    // Read l2, skipping comment lines
    while(std::getline(infile, line)) {
        line.erase(0, line.find_first_not_of(" \t\r\n"));
        if(!line.empty() && line[0] != '#') {
            l2 = line;
    ...
    // Read l3, skipping comment lines
    while(std::getline(infile, line)) {
        line.erase(0, line.find_first_not_of(" \t\r\n"));
        if(!line.empty() && line[0] != '#') {
            l3 = line;
    ...
    // Read l4, skipping comment lines (optional)
    while(std::getline(infile, line)) {
        line.erase(0, line.find_first_not_of(" \t\r\n"));
        if(!line.empty() && line[0] != '#') {
            l4 = line;
	...
	else
	{
		...
		// Set default output calibration to "none" if parsing fails
		outputCalibration[0] = -3;
		valid = false;
		// Don't return here, continue to try reading output resolution
	}


	// l4
	if(!l4.empty() && std::sscanf(l4.c_str(), "%d %d", &w, &h) == 2)
	{
	...
	{
		printf("Out: Failed to Read Output resolution... using input resolution.\n");
		// Use input resolution as default
		w = wOrg;
		h = hOrg;
		valid = false;
    }
    ...
    if(w <= 0 || h <= 0) {
        printf("ERROR: Invalid output resolution %d %d!\n", w, h);
        infile.close();
        return;
    }
