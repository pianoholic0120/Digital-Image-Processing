...
private:
	...
	MinimalImageB* getImageRaw_internal(int id, int unused)
	{
		if(!isZipped)
		{
			...
			{
				printf("WARNING: Image %s has size %d %d, resizing to %d %d (camera.txt specification)\n", 
					files[id].c_str(), img->w, img->h, widthOrg, heightOrg);
				
				// Create resized image using OpenCV
				cv::Mat src(img->h, img->w, CV_8U, img->data);
				cv::Mat dst;
				cv::resize(src, dst, cv::Size(widthOrg, heightOrg), 0, 0, cv::INTER_LINEAR);
				
				// Delete original image
				delete img;
				
				// Create new image with resized data
				img = new MinimalImageB(widthOrg, heightOrg);
				memcpy(img->data, dst.data, widthOrg * heightOrg);
			}
